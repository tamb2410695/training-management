...
import { CONTRACT_TYPE, GENDER, STAFF_STATUS } from "./staffDepartmentEnums";
import { ACCOUNT_ROLES, ACCOUNT_STATUS } from "@/features/accounts";

const Field = buildFields();
export const STAFF_DEPARTMENT_FIELDS = defineFields({
  ...
  identityCard: Field.text("identityCard", "CCCD/CMND")
  ...
  academicRank: Field.text("academicRank", "Học vị")
  ...
  hireDate: Field.date("hireDate", "Ngày tuyển dụng").required().col(6),

  contractType: Field.select("contractType", "Loại hợp đồng", CONTRACT_TYPE)
    .filter("text")
    .required()
    .col(6),
  ...
  departmentNames: Field.text("departmentNames", "Phòng ban")
    .disableApi()
    .disableOnCreate()
    .disableOnUpdate()
    .hideOnForm()
